...
return 0 if e<=Cut
return 1 if e<=Grammar
return 2 if e<=Many
return 3 if e<=Seq
return 4 if e<=Stop
return 5 if e<=Result
return 6 if e<=Act
return 7 if e<=Apply
return 8 if e<=Bind
return 9 if e<=CAct
return 10 if e<=Local
...
bind[20]="int #{bind[7]}=ame_getpos(self);\n"
end
def trans_AmethystCTranslatorcb_13(bind)
bind[20]+=map_index(bind[10]){|i| "#{bind[15]}_#{i+1}: ame_setpos(self,#{bind[7]});if (#{iget("cut")}!=Qnil) {#{iset("cut","Qnil")}; goto #{bind[14]};}\n #{bind[10][i]} \n#{iset("cut","Qnil")};goto #{bind[5]};\n"}*""
end
def trans_AmethystCTranslatorcb_14(bind)
"#{bind[20]}  #{bind[15]}_#{bind[16]+1}:  ame_setpos(self,#{bind[7]}); goto #{bind[14]};\n #{bind[5]}:;\n"
end
...
"int #{bind[7]}=ame_getpos(self); int #{bind[24]}=ame_getlen(self); VALUE #{bind[25]}=ame_getsrc(self); int #{bind[14]}=0;
ame_setsrc(self,#{bget(@src.var)}); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern(\"size\"),0)));
 #{bind[28]}
	goto #{bind[26]};
	#{bind[23]}: #{bind[14]}=1;
	#{bind[26]}: ame_setsrc(self,#{bind[25]}); ame_setpos(self,#{bind[7]}); ame_setlen(self,#{bind[24]});
	if(#{bind[14]}) goto #{@faillabel};
" 
end
...
@rulename=bind[32]; Local.resetnumbering;resetlabels;@locls={}
end
...
h="VALUE #{@grammar}_#{bind[32]}(VALUE self #{map_index(@src.args){|i| ",VALUE a#{i}"}})" 
						@header<<h+";"
...
bind[20]+=bind[39].cases(@src.first) + " #{bind[10]} break;"
end
...
"int #{bind[7]}; while(1){#{bind[7]}=ame_getpos(self); #{bind[10]} if (#{iget("stop")}!=Qnil){{#{bind[7]}=ame_getpos(self);goto #{bind[50]};} } } #{bind[50]}: #{iset("stop","Qnil")};  ame_setpos(self,#{bind[7]}); "
end
...
bind[20]="#{@src.name}.create( {#{@src.vars.map{|l| ":#{l[0]}=>#{rbbget(l)}" }.sort*","} })"
					bind[58]=addcallback(bind[20])
					"it=CALL(#{bind[58]},1,bind);"
...
addcallback(@src.pred ? ["(",bind[63],") || FAIL"] : bind[63])
end
def trans_AmethystCTranslatorcb_48(bind)
"it=CALL(#{bind[58]},1,bind); #{@src.pred ? "FAILTEST(#{@faillabel});" :"" }"
end
...
@faillabel=bind[9]; bind[11] 
end
def trans_AmethystCTranslatorcb_8(bind)
"int #{bind[7]}=ame_getpos(self);\n #{bind[10]} x=1; goto #{bind[5]};  #{bind[6]}: x=0; #{bind[5]}: it=Qnil; ame_setpos(self,#{bind[7]}); if (x==0) goto #{@faillabel};"
end
...
'9ddc8a757e663112c19d2d35a7906809'
end
