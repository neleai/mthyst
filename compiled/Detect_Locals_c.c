...
static VALUE sy_Detect_Locals_src_dot_varn_13a6;
 ...
 memo_struct *mem_Detect_Locals=NULL;VALUE memo_val_Detect_Locals;VALUE profile_report_Detect_Locals(VALUE self){cstruct *ptr; Data_Get_Struct(self,cstruct,ptr); if(ptr->mem){fprintf(profile_report,"memo Detect_Locals::analyze  hit: %i miss: %i ticks: %i\n",((memo_struct *)ptr->mem)->hits[117],((memo_struct *)ptr->mem)->miss[117],((memo_struct *)ptr->mem)->ticks[117]);((memo_struct *)ptr->mem)->hits[117]=0;((memo_struct *)ptr->mem)->miss[117]=0;((memo_struct *)ptr->mem)->ticks[117]=0;} return Qnil;}VALUE Detect_Locals_analyze(VALUE self ){VALUE vals[0]; VALUE it ,__result=Qnil;VALUE bind2=bind_new2(0);   cstruct *ptr; Data_Get_Struct(self,cstruct,ptr);if (ptr->mem==NULL){ptr->mem=mem_Detect_Locals;}time_struct time_old=memo_get(ptr->mem,117,ptr->src,ptr->pos); if (time_old.pos!=-1) {ptr->pos=time_old.pos;return time_old.result;} int oldpos=ptr->pos;it=rb_funcall(self,sy_Detect_Locals__at_locals_eq__545d,1,bind2);  it=Detect_Locals_traverse_item(self ); if (it==failobj){it=failobj;goto memo_fail;}it=rb_funcall(self,sy_Detect_Locals__at_locals_ca83,1,bind2); __result=it;;
...
VALUE Detect_Locals_traverse(VALUE self ){VALUE vals[0]; VALUE it ,_nvars=Qnil,_ivars=Qnil,_autovar=Qnil,_autovar_2=Qnil,_it=Qnil,__result=Qnil;VALUE bind2=bind_new2(2);   cstruct *ptr; Data_Get_Struct(self,cstruct,ptr);ptr->pos=ptr->len;it=rb_ary_new3(0); _nvars=it;;
it=rb_funcall(self,sy_Detect_Locals__lp_src_dot_cla_2024,1,bind2); _ivars=it;;
...
alt1_1:if(!ptr->branches)ptr->discard=ptr->pos; ptr->branches+=2;   it=Detect_Locals_traverse_item(self ); if (it==failobj){it=failobj;goto revert1;} _it=it;;
bind_aset(bind2,1,_nvars);bind_aset(bind2,2,_it);it=rb_funcall(self,sy_Detect_Locals_bind_lb_1_rb__sp__6af0,1,bind2);_nvars=bind_aget(bind2,1);;_it=bind_aget(bind2,2);;
...
alt1_3: ptr->pos=oldpos1;ptr->branches-=1;if (1){it=failobj;goto pass2;};
 accept1:;
                ...
                bind_aset(bind2,1,_ivars);bind_aset(bind2,2,_nvars);it=rb_funcall(self,sy_Detect_Locals_if_sp_bind_lb__1bed,1,bind2);_ivars=bind_aget(bind2,1);;_nvars=bind_aget(bind2,2);; __result=it;;

fail: return it;
}
VALUE Detect_Locals_traverse_item(VALUE self ){VALUE vals[0]; VALUE it ,__result=Qnil,_autovar=Qnil,_autovar_2=Qnil,_autovar_3=Qnil,_ar=Qnil,_it=Qnil,_autovar_4=Qnil;VALUE bind2=bind_new2(2);   cstruct *ptr; Data_Get_Struct(self,cstruct,ptr);switch(FIX2LONG(rb_hash_aref(switchhash_Detect_Locals_2,rb_obj_class(ame_curobj2(ptr))))){case 0/*AmethystAST*/:; switch(FIX2LONG(rb_hash_aref(switchhash_Detect_Locals_3,rb_obj_class(ame_curobj2(ptr))))){case 0/*Local*/:case 1/*Result*/:; int oldpos1=ptr->pos;int cut1=0;
alt1_1:if(!ptr->branches)ptr->discard=ptr->pos; ptr->branches+=3;   it=Detect_Locals_visit(self ); if (it==failobj){it=failobj;goto revert1;} __result=it;;
...
cstruct oldpass1=*ptr; ptr->pos=ptr->len=0; ptr->ary=NULL;
                   ame_setsrc2(self,_autovar);
                     it=Detect_Locals_traverse(self ); if (it==failobj){it=failobj;goto pass1;} _autovar_2=it;;
...
cstruct oldpass2=*ptr; ptr->pos=ptr->len=0; ptr->ary=NULL;
                   ame_setsrc2(self,_autovar);
                     it=Detect_Locals_traverse(self ); if (it==failobj){it=failobj;goto pass2;} _autovar_2=it;;
...
case 1/*Array*/:; switch(FIX2LONG(rb_hash_aref(switchhash_Detect_Locals_4,rb_obj_class(ame_curobj2(ptr))))){case 0/*Local*/:case 1/*Result*/:; int oldpos3=ptr->pos;int cut3=0;
alt3_1:if(!ptr->branches)ptr->discard=ptr->pos; ptr->branches+=2;   it=Detect_Locals_visit(self ); if (it==failobj){it=failobj;goto revert6;} __result=it;;
...
alt4_1:if(!ptr->branches)ptr->discard=ptr->pos; ptr->branches+=2;   it=Detect_Locals_traverse_item(self ); if (it==failobj){it=failobj;goto revert8;} _it=it;;
bind_aset(bind2,1,_ar);bind_aset(bind2,2,_it);it=rb_funcall(self,sy_Detect_Locals_bind_lb_1_rb__lt__7b20,1,bind2);_ar=bind_aget(bind2,1);;_it=bind_aget(bind2,2);;
...
alt5_1:if(!ptr->branches)ptr->discard=ptr->pos; ptr->branches+=2;   it=Detect_Locals_traverse_item(self ); if (it==failobj){it=failobj;goto revert10;} _it=it;;
bind_aset(bind2,1,_ar);bind_aset(bind2,2,_it);it=rb_funcall(self,sy_Detect_Locals_bind_lb_1_rb__lt__7b20,1,bind2);_ar=bind_aget(bind2,1);;_it=bind_aget(bind2,2);;
...
case 2/*Object*/:; switch(FIX2LONG(rb_hash_aref(switchhash_Detect_Locals_5,rb_obj_class(ame_curobj2(ptr))))){case 0/*Local*/:case 1/*Result*/:; int oldpos6=ptr->pos;int cut6=0;
alt6_1:if(!ptr->branches)ptr->discard=ptr->pos; ptr->branches+=2;   it=Detect_Locals_visit(self ); if (it==failobj){it=failobj;goto revert12;} __result=it;;
...
VALUE Detect_Locals_visit(VALUE self ){VALUE vals[0]; VALUE it ,_autovar=Qnil,_x=Qnil,_autovar_2=Qnil,__result=Qnil,_autovar_3=Qnil,_autovar_4=Qnil;VALUE bind2=bind_new2(1);   cstruct *ptr; Data_Get_Struct(self,cstruct,ptr);switch(FIX2LONG(rb_hash_aref(switchhash_Detect_Locals_1,rb_obj_class(ame_curobj2(ptr))))){case 0/*Local*/:; it=ptr->ary[ptr->pos];            ;ptr->pos++; _autovar=it;;
cstruct oldpass1=*ptr; ptr->pos=ptr->len=0; ptr->ary=NULL;
                   ame_setsrc2(self,_autovar);
                     it=AmethystCore_anything(self ); if (it==failobj){it=failobj;goto pass1;} _x=it;;
bind_aset(bind2,1,_x);it=rb_funcall(self,sy_Detect_Locals__at_locals_lt__e5a7,1,bind2);_x=bind_aget(bind2,1);;ptr->pos=ptr->len;it=rb_funcall(self,sy_Detect_Locals_src_25d9,1,bind2); _autovar_2=it;;
                ...
                it=_autovar_2; __result=it;;
 break;
case 1/*Result*/:; it=ptr->ary[ptr->pos];            ;ptr->pos++; _autovar_3=it;;
cstruct oldpass2=*ptr; ptr->pos=ptr->len=0; ptr->ary=NULL;
                   ame_setsrc2(self,_autovar_3);
                   ptr->pos=ptr->len;it=rb_funcall(self,sy_Detect_Locals_src_dot_varn_13a6,1,bind2); _x=it;;
bind_aset(bind2,1,_x);it=rb_funcall(self,sy_Detect_Locals__at_locals_pl__84b1,1,bind2);_x=bind_aget(bind2,1);;it=rb_funcall(self,sy_Detect_Locals_src_25d9,1,bind2); _autovar_4=it;;
                ...
                it=_autovar_4; __result=it;;
 break;
case 2/*Object*/:; if (1){it=failobj;goto fail;} break;
}
fail: return it;
}
void Init_Detect_Locals_c(){ 
 cls_Detect_Locals=rb_define_class("Detect_Locals",rb_const_get(rb_cObject,rb_intern("Traverser_Clone2"))); 
                    ...
                    mem_Detect_Locals=memo_init();memo_val_Detect_Locals=Data_Wrap_Struct(rb_cObject,memo_mark,memo_free,mem_Detect_Locals);rb_global_variable(&memo_val_Detect_Locals);
