...
it=rb_ary_new3(0);char chr1=FIX2INT(CALL(switchcbConstant_Traverser2,1,ame_curobj(self)));  switch(chr1){case 0:; int oldpos1=ame_getpos(self);
alt1_1: ame_setpos(self,oldpos1);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto fail;}
...
ame_setsrc(self,rb_ary_entry(bind,2)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 it=rb_ary_new3(0);
 rb_ary_store(bind,3,it); it=rb_ary_new3(0);int oldpos3; while(1){oldpos3=ame_getpos(self);   it=Constant_Traverser_traverse_item(self ); FAILTEST(break1);
...
ame_setsrc(self,rb_ary_entry(bind,0)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 ...
 rb_ary_store(bind,1,it); it=CALL(visit_Constant_Traversercb_2,1,bind); int oldpos2=ame_getpos(self); int oldlen2=ame_getlen(self); VALUE oldsrc2=ame_getsrc(self); int fail2=0;
ame_setsrc(self,rb_ary_entry(bind,1)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 ...
 rb_ary_store(bind,2,it);   it=Constant_Traverser_eof(self ); FAILTEST(pass2);
	goto success2;
	pass2: fail2=1;
	success2: ame_setsrc(self,oldsrc2); ame_setpos(self,oldpos2); ame_setlen(self,oldlen2);
	if(fail2) goto pass1;
it=CALL(visit_Constant_Traversercb_3,1,bind); it=CALL(visit_Constant_Traversercb_4,1,bind); 
 rb_ary_store(bind,3,it);   it=Constant_Traverser_eof(self ); FAILTEST(pass1);
	goto success1;
	pass1: fail1=1;
...
it=rb_ary_entry(bind,3);
 ...
 rb_ary_store(bind,5,it); int oldpos3=ame_getpos(self); int oldlen3=ame_getlen(self); VALUE oldsrc3=ame_getsrc(self); int fail3=0;
ame_setsrc(self,rb_ary_entry(bind,5)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 int oldpos4; while(1){oldpos4=ame_getpos(self);   it=AmethystCore_anything(self ); FAILTEST(break1); if (IGET(stop)!=Qnil){{oldpos4=ame_getpos(self);goto break1;} } } break1: ISET(stop,Qnil);  ame_setpos(self,oldpos4); it=CALL(visit_Constant_Traversercb_5,1,bind); 
 rb_ary_store(bind,6,it);   it=Constant_Traverser_eof(self ); FAILTEST(pass3);
	goto success3;
	pass3: fail3=1;
...
it=rb_ary_entry(bind,8);
 rb_ary_store(bind,4,it);  break;case 3:; int oldpos7=ame_getpos(self);
  alt1_1:  ame_setpos(self,oldpos7); goto fail;
 accept1:;
...
rb_define_method(cls_Constant_Traverser,"visit",Constant_Traverser_visit,0);
 rb_eval_string("testversionconstant_propagation('abd9ee13325848ab867d53a1d4d4041c')");}
