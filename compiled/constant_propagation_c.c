...
ame_setsrc(self,rb_ary_entry(bind,7/*autovar*/)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 char chr2=FIX2INT(CALL(switchcbConstant_Propagator1,1,ame_curobj(self)));  switch(chr2){case 0:;   it=AmethystCore_anything(self ); FAILTEST(pass4);
 rb_ary_store(bind,8/*autovar*/,it); int oldpos9=ame_getpos(self); int oldlen5=ame_getlen(self); VALUE oldsrc5=ame_getsrc(self); int fail5=0;
ame_setsrc(self,rb_ary_entry(bind,8/*autovar*/)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 int oldpos10; while(1){oldpos10=ame_getpos(self);   it=AmethystCore_anything(self ); FAILTEST(break2); if (IGET(stop)!=Qnil){{oldpos10=ame_getpos(self);goto break2;} } } break2: ISET(stop,Qnil);  ame_setpos(self,oldpos10);  it=CALL(step_Constant_Propagatorcb_5,1,bind);  arg0=it; it=CALL(valof,1 ,arg0); FAILTEST(pass5);
...
it=rb_ary_entry(bind,9/*autovar*/);
 rb_ary_store(bind,10/*autovar*/,it);  break;case 1:;   it=AmethystCore_anything(self ); FAILTEST(pass4);
...
ame_setsrc(self,rb_ary_entry(bind,11/*autovar*/)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 int oldpos12; while(1){oldpos12=ame_getpos(self);   it=AmethystCore_anything(self ); FAILTEST(break3); if (IGET(stop)!=Qnil){{oldpos12=ame_getpos(self);goto break3;} } } break3: ISET(stop,Qnil);  ame_setpos(self,oldpos12); it=CALL(step_Constant_Propagatorcb_6,1,bind); 
...
it=rb_ary_entry(bind,12/*autovar*/);
 rb_ary_store(bind,10/*autovar*/,it);  break;case 2:;   it=Constant_Propagator_fails(self ); FAILTEST(pass4); break;}  it=Constant_Propagator_eof(self ); FAILTEST(pass4);
	goto success4;
	pass4: fail4=1;
...
rb_define_method(cls_Constant_Traverser,"visit",Constant_Traverser_visit,0);
 rb_eval_string("testversionconstant_propagation('6fcbe2a2fafd05ef9e60635e82f4fca6')");}
