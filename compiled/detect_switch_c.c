...
ame_setsrc(self,BGET(autovar_2)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 ...
 BSET(ary2_1,it); it=rb_ary_new3(0);
 BSET(autovar_1,it); int oldpos2; while(1){oldpos2=ame_getpos(self);   it=CALL(traverse_item,0 ); FAILTEST(break1);
 BSET(e_1,it); it=CALL(visit_Detect_Switchcb_3,1,bind); FAILTEST(break1);it=CALL(visit_Detect_Switchcb_4,1,bind); 
 BSET(autovar_3,it); it=CALL(visit_Detect_Switchcb_5,1,bind);  if (IGET(stop)!=Qnil){{oldpos2=ame_getpos(self);goto break1;} } } break1: ISET(stop,Qnil);  ame_setpos(self,oldpos2); it=BGET(autovar_1);it=CALL(visit_Detect_Switchcb_6,1,bind); 
	if (CALL(eof,0)==failobj) goto pass1;
	goto success1;
	pass1: fail1=1;
...
ame_setsrc(self,BGET(autovar_2)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 ...
 BSET(ary2_1,it); it=CALL(visit_Detect_ClasSwitchcb_3,1,bind); it=rb_ary_new3(0);
 BSET(autovar_1,it); int oldpos2; while(1){oldpos2=ame_getpos(self);   it=CALL(traverse_item,0 ); FAILTEST(break1);
 BSET(e_1,it); it=CALL(visit_Detect_ClasSwitchcb_4,1,bind); FAILTEST(break1);it=CALL(visit_Detect_ClasSwitchcb_5,1,bind); 
 BSET(autovar_3,it); it=CALL(visit_Detect_ClasSwitchcb_6,1,bind);  if (IGET(stop)!=Qnil){{oldpos2=ame_getpos(self);goto break1;} } } break1: ISET(stop,Qnil);  ame_setpos(self,oldpos2); it=BGET(autovar_1);it=CALL(visit_Detect_ClasSwitchcb_7,1,bind); 
	if (CALL(eof,0)==failobj) goto pass1;
	goto success1;
	pass1: fail1=1;
	success1: ame_setsrc(self,oldsrc1); ame_setpos(self,oldpos1); ame_setlen(self,oldlen1);
