...
def Detect_Contextual_Arguments__append_lp__6b01(bind)
_append(@arguments,bind[1])
end
def Detect_Contextual_Arguments__at_argumen_a3fb(bind)
@arguments.uniq.sort
end
def Detect_Contextual_Arguments__at_argumen_c331(bind)
@arguments=[]
end
def Detect_Contextual_Arguments__lp_src_dot_cla_2024(bind)
(src.class.attributes).map{|v| src.instance_variable_get("@"+v.to_s) }
end
def Detect_Contextual_Arguments_bind_lb_1_rb__lt__7b20(bind)
bind[1]<<bind[2]
end
def Detect_Contextual_Arguments_bind_lb_1_rb__sp__6af0(bind)
bind[1] << bind[2]
end
def Detect_Contextual_Arguments_if_sp_bind_lb__1bed(bind)
if bind[1]==bind[2]
						  src
						else
