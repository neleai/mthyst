...
it=rb_ary_entry(bind,3);
 rb_ary_store(bind,4,it);  break;case 1:; int oldpos4=ame_getpos(self);
alt1_1: ame_setpos(self,oldpos4);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto fail;}
   it=AmethystCore_anything(self ); FAILTEST(alt1_2);
 rb_ary_store(bind,5,it); int oldpos5=ame_getpos(self); int oldlen3=ame_getlen(self); VALUE oldsrc3=ame_getsrc(self); int fail3=0;
ame_setsrc(self,rb_ary_entry(bind,5)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
   it=AmethystCore_anything(self ); FAILTEST(pass3);
 rb_ary_store(bind,6,it); it=CALL(visit_Seq_Or_Optimizercb_2,1,bind); FAILTEST(pass3);it=CALL(visit_Seq_Or_Optimizercb_3,1,bind);   it=AmethystCore_anything(self ); FAILTEST(pass3);
 rb_ary_store(bind,7,it); it=CALL(visit_Seq_Or_Optimizercb_4,1,bind); FAILTEST(pass3);it=CALL(visit_Seq_Or_Optimizercb_5,1,bind); 
 rb_ary_store(bind,8,it);   it=Seq_Or_Optimizer_eof(self ); FAILTEST(pass3);
	goto success3;
	pass3: fail3=1;
	success3: ame_setsrc(self,oldsrc3); ame_setpos(self,oldpos5); ame_setlen(self,oldlen3);
...
ISET(cut,Qnil);goto accept1;
alt1_2: ame_setpos(self,oldpos4);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto fail;}
   it=AmethystCore_anything(self ); FAILTEST(alt1_3);
 rb_ary_store(bind,9,it); int oldpos6=ame_getpos(self); int oldlen4=ame_getlen(self); VALUE oldsrc4=ame_getsrc(self); int fail4=0;
ame_setsrc(self,rb_ary_entry(bind,9)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 ...
 rb_ary_store(bind,6,it); it=CALL(visit_Seq_Or_Optimizercb_6,1,bind); FAILTEST(pass4);  it=AmethystCore_anything(self ); FAILTEST(pass4);
 rb_ary_store(bind,7,it); it=CALL(visit_Seq_Or_Optimizercb_7,1,bind); FAILTEST(pass4);it=CALL(visit_Seq_Or_Optimizercb_8,1,bind); 
...
it=rb_ary_entry(bind,10);
 rb_ary_store(bind,4,it);  
ISET(cut,Qnil);goto accept1;
  alt1_3:  ame_setpos(self,oldpos4); goto fail;
 accept1:;
 break;case 2:;   it=AmethystCore_anything(self ); FAILTEST(fail);
 rb_ary_store(bind,11,it); int oldpos7=ame_getpos(self); int oldlen5=ame_getlen(self); VALUE oldsrc5=ame_getsrc(self); int fail5=0;
ame_setsrc(self,rb_ary_entry(bind,11)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
   it=Seq_Or_Optimizer_traverse(self ); FAILTEST(pass5);
...
ame_setsrc(self,rb_ary_entry(bind,12)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 int oldpos9; while(1){oldpos9=ame_getpos(self);   it=AmethystCore_anything(self ); FAILTEST(break2); if (IGET(stop)!=Qnil){{oldpos9=ame_getpos(self);goto break2;} } } break2: ISET(stop,Qnil);  ame_setpos(self,oldpos9); it=CALL(visit_Seq_Or_Optimizercb_9,1,bind); 
 rb_ary_store(bind,13,it);   it=Seq_Or_Optimizer_eof(self ); FAILTEST(pass6);
	goto success6;
	pass6: fail6=1;
	success6: ame_setsrc(self,oldsrc6); ame_setpos(self,oldpos8); ame_setlen(self,oldlen6);
	if(fail6) goto pass5;
it=rb_ary_entry(bind,13);
 rb_ary_store(bind,14,it);   it=Seq_Or_Optimizer_eof(self ); FAILTEST(pass5);
	goto success5;
	pass5: fail5=1;
	success5: ame_setsrc(self,oldsrc5); ame_setpos(self,oldpos7); ame_setlen(self,oldlen5);
	if(fail5) goto fail;
it=rb_ary_entry(bind,14);
 rb_ary_store(bind,4,it);  break;default:;   it=Seq_Or_Optimizer_fails(self ); FAILTEST(fail); break;}it=rb_ary_entry(bind,4);
return it;
fail: return failobj; }
...
rb_define_method(cls_Seq_Or_Optimizer,"visit",Seq_Or_Optimizer_visit,0);
 rb_eval_string("testversionoptimizer_and_or('698e6e511cb4ee2e561e1de1e4ad9129')");}
