...
it=rb_ary_entry(bind,6);
 rb_ary_store(bind,7,it); it=CALL(itrans_Seq_Or_Optimizercb_5,1,bind);  if (IGET(stop)!=Qnil){{oldpos6=ame_getpos(self);goto break2;} } } break2: ISET(stop,Qnil);  ame_setpos(self,oldpos6); it=rb_ary_entry(bind,4);
...
ISET(cut,Qnil);goto accept1;
 ...
 rb_ary_store(bind,10,it);  break;default: goto break1;}it=CALL(itrans_Seq_Or_Optimizercb_8,1,bind);  if (IGET(stop)!=Qnil){{oldpos1=ame_getpos(self);goto break1;} } } break1: ISET(stop,Qnil);  ame_setpos(self,oldpos1); it=rb_ary_entry(bind,0);
...
ame_setsrc(self,rb_ary_entry(bind,0)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
   it=Seq_Or_Optimizer_traverse(self ); FAILTEST(pass1);it=CALL(visit_Seq_Or_Optimizercb_2,1,bind); 
...
alt1_1: ame_setpos(self,oldpos2);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto fail;}
  it=CALL(visit_Seq_Or_Optimizercb_3,1,bind);  arg0=it; it=Seq_Or_Optimizer_clas(self ,arg0); FAILTEST(alt1_2);
 rb_ary_store(bind,3,it); int oldpos3=ame_getpos(self); int oldlen2=ame_getlen(self); VALUE oldsrc2=ame_getsrc(self); int fail2=0;
ame_setsrc(self,rb_ary_entry(bind,3)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
   it=AmethystCore_anything(self ); FAILTEST(pass2);
 rb_ary_store(bind,4,it); it=CALL(visit_Seq_Or_Optimizercb_4,1,bind); FAILTEST(pass2);it=CALL(visit_Seq_Or_Optimizercb_5,1,bind);   it=AmethystCore_anything(self ); FAILTEST(pass2);
 rb_ary_store(bind,5,it); it=CALL(visit_Seq_Or_Optimizercb_6,1,bind); FAILTEST(pass2);it=CALL(visit_Seq_Or_Optimizercb_7,1,bind); 
...
alt1_2: ame_setpos(self,oldpos2);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto fail;}
  it=CALL(visit_Seq_Or_Optimizercb_8,1,bind);  arg0=it; it=Seq_Or_Optimizer_clas(self ,arg0); FAILTEST(alt1_3);
...
ame_setsrc(self,rb_ary_entry(bind,7)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
   it=AmethystCore_anything(self ); FAILTEST(pass3);
 rb_ary_store(bind,4,it); it=CALL(visit_Seq_Or_Optimizercb_9,1,bind); FAILTEST(pass3);  it=AmethystCore_anything(self ); FAILTEST(pass3);
 rb_ary_store(bind,5,it); it=CALL(visit_Seq_Or_Optimizercb_10,1,bind); FAILTEST(pass3);it=CALL(visit_Seq_Or_Optimizercb_11,1,bind); 
 rb_ary_store(bind,8,it);   it=Seq_Or_Optimizer_eof(self ); FAILTEST(pass3);
	goto success3;
	pass3: fail3=1;
...
ISET(cut,Qnil);goto accept1;
 ...
 accept1:;
 break;case 2:;  it=CALL(visit_Seq_Or_Optimizercb_12,1,bind);  arg0=it; it=Seq_Or_Optimizer_clas(self ,arg0); FAILTEST(fail);
...
ame_setsrc(self,rb_ary_entry(bind,9)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
   it=Seq_Or_Optimizer_traverse(self ); FAILTEST(pass4);it=CALL(visit_Seq_Or_Optimizercb_13,1,bind); 
...
rb_define_method(cls_Seq_Or_Optimizer,"visit",Seq_Or_Optimizer_visit,0);
 rb_eval_string("testversion('c3c93400817b47ca4e0dd921a8a02404')");}
