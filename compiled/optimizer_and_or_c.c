...
alt1_1: ame_setpos(self,oldpos2);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto break1;}
 ...
 BSET(i_1,it); it=CALL(itrans_Seq_Or_Optimizercb_2,1,bind); FAILTEST(alt1_2);it=BGET(i_1);
 BSET(_result_1,it); it=BGET(_result_1);
...
ame_setsrc(self,BGET(autovar_2)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 ...
 BSET(autovar_3,it); it=CALL(itrans_Seq_Or_Optimizercb_4,1,bind); int oldpos4=ame_getpos(self); int oldlen2=ame_getlen(self); VALUE oldsrc2=ame_getsrc(self); int fail2=0;
...
ame_setsrc(self,BGET(autovar_4)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 it=rb_ary_new3(0);
 BSET(autovar_5,it); int oldpos6; while(1){oldpos6=ame_getpos(self); it=CALL(itrans_Seq_Or_Optimizercb_5,1,bind); 
 BSET(cls_1,it);   it=AmethystCore_anything(self ); FAILTEST(break2);
 BSET(i_1,it); it=CALL(itrans_Seq_Or_Optimizercb_6,1,bind); FAILTEST(break2);it=BGET(i_1);
 BSET(_result_1,it); it=BGET(_result_1);
...
it=CALL(itrans_Seq_Or_Optimizercb_7,1,bind);  if (IGET(stop)!=Qnil){{oldpos6=ame_getpos(self);goto break2;} } } break2: ISET(stop,Qnil);  ame_setpos(self,oldpos6); it=BGET(autovar_5);
 BSET(rules_1,it); 
	...
	if(fail2) goto pass1;
it=CALL(itrans_Seq_Or_Optimizercb_8,1,bind); it=CALL(itrans_Seq_Or_Optimizercb_9,1,bind); 
 BSET(autovar_8,it); 
...
ISET(cut,Qnil);goto accept1;
 ...
 accept1:;
it=CALL(itrans_Seq_Or_Optimizercb_10,1,bind);  if (IGET(stop)!=Qnil){{oldpos1=ame_getpos(self);goto break1;} } } break1: ISET(stop,Qnil);  ame_setpos(self,oldpos1); it=BGET(autovar_1);
...
ame_setsrc(self,BGET(autovar_3)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
   it=AmethystCore_anything(self ); FAILTEST(pass3);
 BSET(name_1,it); it=CALL(visit_Seq_Or_Optimizercb_11,1,bind); FAILTEST(pass3);it=CALL(visit_Seq_Or_Optimizercb_12,1,bind);   it=AmethystCore_anything(self ); FAILTEST(pass3);
 BSET(arg_1,it); it=CALL(visit_Seq_Or_Optimizercb_13,1,bind); FAILTEST(pass3);it=CALL(visit_Seq_Or_Optimizercb_14,1,bind); it=CALL(visit_Seq_Or_Optimizercb_15,1,bind); 
 BSET(_result_2,it); 
	if (CALL(eof,0)==failobj) goto pass3;
	goto success3;
	pass3: fail3=1;
...
ISET(cut,Qnil);goto accept1;
alt1_4: ame_setpos(self,oldpos1);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto fail;}
 it=CALL(visit_Seq_Or_Optimizercb_16,1,bind); 
 BSET(cls_1,it);   it=AmethystCore_anything(self ); FAILTEST(alt1_5);
 BSET(i_1,it); it=CALL(visit_Seq_Or_Optimizercb_17,1,bind); FAILTEST(alt1_5);it=BGET(i_1);
 BSET(_result_1,it); it=BGET(_result_1);
...
ame_setsrc(self,BGET(autovar_4)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 ...
 BSET(name_1,it); it=CALL(visit_Seq_Or_Optimizercb_18,1,bind); FAILTEST(pass4);  it=AmethystCore_anything(self ); FAILTEST(pass4);
 BSET(arg_1,it); it=CALL(visit_Seq_Or_Optimizercb_19,1,bind); FAILTEST(pass4);it=CALL(visit_Seq_Or_Optimizercb_20,1,bind); 
 BSET(_result_2,it); 
	if (CALL(eof,0)==failobj) goto pass4;
	goto success4;
	pass4: fail4=1;
