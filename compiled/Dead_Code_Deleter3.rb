...
def Dead_Code_Deleter3__at__contex_5f56(bind)
@_context_arguments||={};bind[1]=@_context_arguments;bind[2]=@_context_arguments.dup
end
def Dead_Code_Deleter3__at__contex_cdb3(bind)
@_context_arguments=bind[1];bind[2]
end
def Dead_Code_Deleter3__at__contex_d6d5(bind)
...
def Dead_Code_Deleter3__lp_src_dot_cla_2024(bind)
(src.class.attributes).map{|v| src.instance_variable_get("@"+v.to_s) }
end
def Dead_Code_Deleter3_bind_lb_1_rb__lb__146c(bind)
bind[1][:bound]||=bind[1][:reachable][src.name]
end
def Dead_Code_Deleter3_bind_lb_1_rb__lb__1f9e(bind)
bind[1][:reachable]=src.reachable
end
def Dead_Code_Deleter3_bind_lb_1_rb__lb__8eca(bind)
...
def Dead_Code_Deleter3_d_eq_src_dot_du_2b46(bind)
d=src.dup;#d.bound=bind[1][:bound];
                      d.normalize
end
def Dead_Code_Deleter3_d_eq_src_dot_du_d571(bind)
...
def Dead_Code_Deleter3_src_dot_body_519e(bind)
src.body
end
def Dead_Code_Deleter3_src_dot_body_aaae(bind)
src.body=bind[1];src
end
...
def Dead_Code_Deleter3_src_dot_to_5e99(bind)
src.to
end
