...
alt3_1: ame_setpos(self,oldpos3);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto alt2_3;}
 it=CALL(factor_Left_Factorcb_3,1,bind); ISET(cut,Qtrue);  it=Amethyst_fails(self ); FAILTEST(alt3_2); 
...
alt3_1: ame_setpos(self,oldpos5);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto alt2_3;}
 it=CALL(visit_Left_Factorcb_5,1,bind); ISET(cut,Qtrue);  it=Amethyst_fails(self ); FAILTEST(alt3_2); 
ISET(cut,Qnil);goto accept3;
...
rb_define_method(cls_Left_Factor,"visit",Left_Factor_visit,0);
 rb_eval_string("testversionleft_factor('e209780fc21228da95caa2e58f92d18f')");}
