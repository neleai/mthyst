...
it=CALL(spaces_Amethystcb_1,1,bind); 
 ...
 rb_ary_store(bind,1/*c*/,it); it=CALL(lower_Amethystcb_2,1,bind); FAILTEST(fail);it=rb_ary_entry(bind,1/*c*/);
 rb_ary_store(bind,2/*_result*/,it); it=rb_ary_entry(bind,2/*_result*/);
 rb_ary_store(bind,3/*c*/,it); it=CALL(lower_Amethystcb_3,1,bind); FAILTEST(fail);it=rb_ary_entry(bind,3/*c*/);
...
alt1_1: ame_setpos(self,oldpos1);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto fail;}
 ...
 rb_ary_store(bind,2/*autovar*/,it); it=CALL(listOf_Amethystcb_1,1,bind); it=rb_ary_new3(0);
 ...
 rb_ary_store(bind,5/*autovar*/,it); it=CALL(listOf_Amethystcb_2,1,bind);  if (IGET(stop)!=Qnil){{oldpos2=ame_getpos(self);goto break1;} } } break1: ISET(stop,Qnil);  ame_setpos(self,oldpos2); it=rb_ary_entry(bind,3/*autovar*/);
 rb_ary_store(bind,6/*autovar*/,it); it=CALL(listOf_Amethystcb_3,1,bind); it=rb_ary_entry(bind,0/*f*/);
...
it=rb_ary_new3(0);
 rb_ary_store(bind,0/*autovar*/,it); int oldpos1; while(1){oldpos1=ame_getpos(self); it=CALL(spaces_Amethystcb_1,1,bind); 
 rb_ary_store(bind,1/*regex*/,it);   it=AmethystCore_anything(self ); FAILTEST(break1);
 rb_ary_store(bind,2/*c*/,it); it=CALL(spaces_Amethystcb_2,1,bind); FAILTEST(break1);it=rb_ary_entry(bind,2/*c*/);
 rb_ary_store(bind,3/*_result*/,it); it=rb_ary_entry(bind,3/*_result*/);
 rb_ary_store(bind,4/*c*/,it); it=CALL(spaces_Amethystcb_3,1,bind); FAILTEST(break1);it=rb_ary_entry(bind,4/*c*/);
 rb_ary_store(bind,5/*_result*/,it); it=rb_ary_entry(bind,5/*_result*/);
 rb_ary_store(bind,6/*_result*/,it); it=rb_ary_entry(bind,6/*_result*/);
 rb_ary_store(bind,7/*autovar*/,it); it=CALL(spaces_Amethystcb_4,1,bind);  if (IGET(stop)!=Qnil){{oldpos1=ame_getpos(self);goto break1;} } } break1: ISET(stop,Qnil);  ame_setpos(self,oldpos1); it=rb_ary_entry(bind,0/*autovar*/);
...
rb_define_method(cls_Amethyst,"xdigit",Amethyst_xdigit,0);
 rb_eval_string("testversionamethyst('80003705f177af2bdef236ad1f54edb4')");}
