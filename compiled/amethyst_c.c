...
unsigned char chr1=*ame_curstr(self);  switch(chr1){case UC('\000') ... UC('`'):;case UC('d') ... UC('f'):;case UC('j') ... UC('\377'):;  it=CALL(foo_Amethystcb_1,1,bind);  arg0=it; it=Amethyst_regch(self ,arg0); FAILTEST(fail);
 rb_ary_store(bind,0/*_result*/,it);  break;case UC('g') ... UC('i'):; int oldpos1=ame_getpos(self);
alt1_1: ame_setpos(self,oldpos1);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto fail;}
...
rb_define_method(cls_Amethyst,"xdigit",Amethyst_xdigit,0);
 rb_eval_string("testversionamethyst('7d9728f4391914b0bb779f849a633484')");}
