...
return it;
fail: return failobj; }
VALUE Remap_Acts_visit(VALUE self ){VALUE vals[0]; VALUE bind=rb_ary_new2(16);  int x;VALUE arg0,arg1,arg2,arg3;VALUE it;
...
ame_setsrc(self,rb_ary_entry(bind,0)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 ...
 rb_ary_store(bind,1,it); int oldpos2=ame_getpos(self); int oldlen2=ame_getlen(self); VALUE oldsrc2=ame_getsrc(self); int fail2=0;
ame_setsrc(self,rb_ary_entry(bind,1)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 it=rb_ary_new3(0);int oldpos3; while(1){oldpos3=ame_getpos(self);   it=AmethystCore_anything(self ); FAILTEST(break1); if (IGET(stop)!=Qnil){{oldpos3=ame_getpos(self);goto break1;} } } break1: ISET(stop,Qnil);  ame_setpos(self,oldpos3); it=CALL(visit_Remap_Actscb_1,1,bind); 
...
ame_setsrc(self,rb_ary_entry(bind,5)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 it=rb_ary_new3(0);
 rb_ary_store(bind,6,it); int oldpos5; while(1){oldpos5=ame_getpos(self);   it=Remap_Acts_traverse_item(self ); FAILTEST(break2);
 rb_ary_store(bind,7,it); it=CALL(visit_Remap_Actscb_2,1,bind);  if (IGET(stop)!=Qnil){{oldpos5=ame_getpos(self);goto break2;} } } break2: ISET(stop,Qnil);  ame_setpos(self,oldpos5); it=rb_ary_entry(bind,6);
 rb_ary_store(bind,8,it); it=CALL(visit_Remap_Actscb_3,1,bind); 
 rb_ary_store(bind,9,it);   it=Remap_Acts_eof(self ); FAILTEST(pass3);
	goto success3;
	pass3: fail3=1;
	success3: ame_setsrc(self,oldsrc3); ame_setpos(self,oldpos4); ame_setlen(self,oldlen3);
	if(fail3) goto fail;
it=rb_ary_entry(bind,9);
 ...
 rb_ary_store(bind,10,it); int oldpos6=ame_getpos(self); int oldlen4=ame_getlen(self); VALUE oldsrc4=ame_getsrc(self); int fail4=0;
ame_setsrc(self,rb_ary_entry(bind,10)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 it=rb_ary_new3(0);
 rb_ary_store(bind,11,it); int oldpos7; while(1){oldpos7=ame_getpos(self);   it=Remap_Acts_traverse_item(self ); FAILTEST(break3);
 rb_ary_store(bind,12,it); it=CALL(visit_Remap_Actscb_4,1,bind);  if (IGET(stop)!=Qnil){{oldpos7=ame_getpos(self);goto break3;} } } break3: ISET(stop,Qnil);  ame_setpos(self,oldpos7); it=rb_ary_entry(bind,11);
 rb_ary_store(bind,8,it); it=CALL(visit_Remap_Actscb_5,1,bind); 
 rb_ary_store(bind,13,it);   it=Remap_Acts_eof(self ); FAILTEST(pass4);
	goto success4;
	pass4: fail4=1;
...
it=rb_ary_entry(bind,13);
 ...
 rb_ary_store(bind,14,it); int oldpos8=ame_getpos(self); int oldlen5=ame_getlen(self); VALUE oldsrc5=ame_getsrc(self); int fail5=0;
ame_setsrc(self,rb_ary_entry(bind,14)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 it=rb_ary_new3(0);int oldpos9; while(1){oldpos9=ame_getpos(self);   it=AmethystCore_anything(self ); FAILTEST(break4); if (IGET(stop)!=Qnil){{oldpos9=ame_getpos(self);goto break4;} } } break4: ISET(stop,Qnil);  ame_setpos(self,oldpos9); it=CALL(visit_Remap_Actscb_6,1,bind); 
 rb_ary_store(bind,15,it);   it=Remap_Acts_eof(self ); FAILTEST(pass5);
	goto success5;
	pass5: fail5=1;
	success5: ame_setsrc(self,oldsrc5); ame_setpos(self,oldpos8); ame_setlen(self,oldlen5);
	if(fail5) goto fail;
it=rb_ary_entry(bind,15);
 rb_ary_store(bind,4,it);  break;case 4:; int oldpos10=ame_getpos(self);
  alt1_1:  ame_setpos(self,oldpos10); goto fail;
 accept1:;
...
rb_define_method(cls_Remap_Acts,"visit",Remap_Acts_visit,0);
 rb_eval_string("testversiondetect_variables2('00b5b7d87e32a5c6307543566c96bc82')");}
