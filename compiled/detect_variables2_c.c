...
return it;
fail: return failobj; }
VALUE Remap_Acts_visit(VALUE self ){VALUE vals[0]; VALUE bind=rb_ary_new2(25);  int x;VALUE arg0,arg1,arg2,arg3;VALUE it;
...
ame_setsrc(self,rb_ary_entry(bind,23)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 it=rb_ary_new3(0);int oldpos12; while(1){oldpos12=ame_getpos(self);   it=AmethystCore_anything(self ); FAILTEST(break5); if (IGET(stop)!=Qnil){{oldpos12=ame_getpos(self);goto break5;} } } break5: ISET(stop,Qnil);  ame_setpos(self,oldpos12); it=CALL(visit_Remap_Actscb_12,1,bind); 
 rb_ary_store(bind,24,it);   it=Remap_Acts_eof(self ); FAILTEST(pass7);
	goto success7;
	pass7: fail7=1;
	success7: ame_setsrc(self,oldsrc7); ame_setpos(self,oldpos11); ame_setlen(self,oldlen7);
	if(fail7) goto fail;
it=rb_ary_entry(bind,24);
 rb_ary_store(bind,5,it);  break;case 6:; int oldpos13=ame_getpos(self);
  alt1_1:  ame_setpos(self,oldpos13); goto fail;
 accept1:;
...
rb_define_method(cls_Remap_Acts,"visit",Remap_Acts_visit,0);
 rb_eval_string("testversiondetect_variables2('7724ecc63c82c43143d2ee7ed04d5531')");}
