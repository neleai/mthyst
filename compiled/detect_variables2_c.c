...
ame_setsrc(self,rb_ary_entry(bind,18)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 ...
 rb_ary_store(bind,20,it); it=CALL(visit_Analyze_Variables2cb_10,1,bind);  if (IGET(stop)!=Qnil){{oldpos13=ame_getpos(self);goto break3;} } } break3: ISET(stop,Qnil);  ame_setpos(self,oldpos13); it=rb_ary_entry(bind,19);
 rb_ary_store(bind,21,it); it=CALL(visit_Analyze_Variables2cb_11,1,bind); 
...
ame_setsrc(self,rb_ary_entry(bind,23)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 ...
 rb_ary_store(bind,25,it); it=CALL(visit_Analyze_Variables2cb_12,1,bind);  if (IGET(stop)!=Qnil){{oldpos15=ame_getpos(self);goto break4;} } } break4: ISET(stop,Qnil);  ame_setpos(self,oldpos15); it=rb_ary_entry(bind,24);
 rb_ary_store(bind,21,it); it=CALL(visit_Analyze_Variables2cb_13,1,bind); 
...
alt1_4: ame_setpos(self,oldpos16);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto fail;}
 ...
 rb_ary_store(bind,33,it);  
ISET(cut,Qnil);goto accept1;
  alt1_5:  ame_setpos(self,oldpos16); goto fail;
 accept1:;
...
ame_setsrc(self,rb_ary_entry(bind,33)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
 it=rb_ary_new3(0);int oldpos18; while(1){oldpos18=ame_getpos(self);   it=AmethystCore_anything(self ); FAILTEST(break6); if (IGET(stop)!=Qnil){{oldpos18=ame_getpos(self);goto break6;} } } break6: ISET(stop,Qnil);  ame_setpos(self,oldpos18); it=CALL(visit_Remap_Actscb_23,1,bind); 
...
rb_define_method(cls_Remap_Acts,"visit",Remap_Acts_visit,0);
 rb_eval_string("testversiondetect_variables2('9e2a412476682c02e6742fd8af63fcdd')");}
