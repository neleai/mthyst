...
def predicate_Detect_Switchcb_10(bind)
nary=bind[1].ary
...
@cases=@cases.map{|c| [c&bind[12],c&~bind[12]]}.flatten.select{|x| x.ary.size>0}.uniq_by{|x|x.ary.inspect} 
end
...
def traverse_Detect_Switchcb_8(bind)
_append(bind[8],bind[13])
end
def traverse_Detect_Switchcb_9(bind)
if bind[5]
...
predicate(bind[0],bind[1],bind[24])
end
def predicate_Detect_ClasSwitchcb_14(bind)
_Bind(bind[19],bind[25])
end
...
def traverse_Detect_ClasSwitchcb_8(bind)
_append(bind[8],bind[13])
end
def traverse_Detect_ClasSwitchcb_9(bind)
if bind[5]
             @changed=true;bind[4].normalize
           else
            @changed=bind[2]
...
'8ae245e0057ea565487a488d3e3c6452'
end
