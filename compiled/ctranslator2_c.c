...
ame_setsrc(self,BGET(autovar_35)); ame_setpos(self,0); ame_setlen(self,FIX2INT(rb_funcall(ame_getsrc(self),rb_intern("size"),0)));
  it=CALL(trans_AmethystCTranslatorcb_99,1,bind);  arg0=it; it=CALL(label,1 ,arg0); FAILTEST(pass23);
 BSET(pass_1,it);  it=CALL(trans_AmethystCTranslatorcb_100,1,bind);  arg0=it; it=CALL(label,1 ,arg0); FAILTEST(pass23);
 BSET(oldpos_1,it);  it=CALL(trans_AmethystCTranslatorcb_101,1,bind);  arg0=it; it=CALL(label,1 ,arg0); FAILTEST(pass23);
 BSET(oldlen_1,it);  it=CALL(trans_AmethystCTranslatorcb_102,1,bind);  arg0=it; it=CALL(label,1 ,arg0); FAILTEST(pass23);
 BSET(oldsrc_1,it);  it=CALL(trans_AmethystCTranslatorcb_103,1,bind);  arg0=it; it=CALL(label,1 ,arg0); FAILTEST(pass23);
 BSET(success_1,it);  it=CALL(trans_AmethystCTranslatorcb_104,1,bind);  arg0=it; it=CALL(label,1 ,arg0); FAILTEST(pass23);
 BSET(fail_1,it); it=BGET(pass_1);
 BSET(word_1,it); it=CALL(trans_AmethystCTranslatorcb_105,1,bind); 
 BSET(rwo_1,it); it=CALL(trans_AmethystCTranslatorcb_106,1,bind); it=CALL(trans_AmethystCTranslatorcb_107,1,bind); 
 BSET(autovar_36,it); it=CALL(trans_AmethystCTranslatorcb_108,1,bind); int oldpos33=ame_getpos(self); int oldlen24=ame_getlen(self); VALUE oldsrc24=ame_getsrc(self); int fail24=0;
...
it=BGET(_result_2);
 BSET(x_1,it); it=CALL(trans_AmethystCTranslatorcb_109,1,bind); it=BGET(_result_4);it=CALL(trans_AmethystCTranslatorcb_110,1,bind); 
 BSET(_result_2,it); 
