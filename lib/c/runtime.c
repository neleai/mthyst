...
ID s_to_a;
static VALUE ame_setlenrb(VALUE self, VALUE val){
  return INT2FIX(ame_setlen(self, FIX2INT(val)));
}
static VALUE ame_setposrb(VALUE self, VALUE val){
  return INT2FIX(ame_setpos(self, FIX2INT(val)));
}
static VALUE ame_getlenrb(VALUE self){
  return INT2FIX(ame_getlen(self));
}

VALUE ame_setsrc2(VALUE self, VALUE val){
  cstruct *ptr;  Data_Get_Struct(self, cstruct, ptr);
  ptr->src = val;
  if (TYPE(ptr->src) == T_STRING) {
      ptr->str = RSTRING_PTR(ptr->src); ptr->len = RSTRING_LEN(ptr->src);
    } else { VALUE ary;
...
VALUE AmethystCore__seq(VALUE self, VALUE str){
  ...
  if (TYPE(ptr->src) == T_STRING) {
      if (strncmp(ptr->str + ptr->pos, RSTRING_PTR(str), len)) return failobj; else {
          ptr->pos += len;
          return Qnil;
        }
    }else
...
void ame_mark(cstruct *ptr){
  rb_gc_mark(ptr->memgc);
}
void ame_free(cstruct *ptr){
  free(ptr);
}
...
void bind_cache_mark(bind_cache *b){
  int i;
