...
static int memo_hash(int rule, int src, int pos, int cap){
  return (src + rule * pos) & (cap - 1);
}
...
static void memo_add(memo_struct *m, int rule, VALUE src, int pos, VALUE val, int newpos, time_struct time_old){
  ...
  if (2 * m->size > m->capacity) {
      ...
      for (i = 0; i < m->capacity;
           i++) if (m->els[i].rule && !(m->els[i].src == src && m->els[i].pos < time_old.discard)) resized[memo_hash(m->els[i].rule, m->els[i].src, m->els[i].pos, 2 * m->capacity)] = m->els[i];
      free(m->els);
      m->els = resized;
...
void memo_mark(memo_struct *m){  /*we want weak link to src so we dont mark it*/
  int i;
  for (i = 0; i < CACHE_SIZE; i++) if (m->cache[i].rule) {
        rb_gc_mark(m->els[i].src); rb_gc_mark(m->cache[i].val);
      }
  for (i = 0; i < m->capacity; i++) if (m->els[i].rule) {
        rb_gc_mark(m->els[i].src); rb_gc_mark(m->els[i].val);
      }
}
void memo_free(memo_struct *m){
  free(m->els); free(m->hits); free(m->miss); free(m);
}
