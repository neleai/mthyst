...
( self['body']=Seq[{:ary=>((@locals- self['args']).map{|v| Set[{:name=>v,:expr=>Act["nil"]}]}+[ self['body']]) }])
...
_result_1 = (("#{name_1}.create(#{argss_1} {#{vars_1.map{|l| ":#{l[0]}=>#{l[0]}_#{@varhash[l[0]][l[1]]}" }*","} })")) },proc{autovar_25 = ((it=(clas(Key));next FAIL if it==FAIL;it))
