...
_result_1 = (Grammar[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_15 = ((it=(clas(Rule));next FAIL if it==FAIL;it))
...
_result_1 = (Rule[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_16 = ((it=(clas(Or));next FAIL if it==FAIL;it))
...
_result_1 = (Or[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_18 = ((it=(clas(Seq));next FAIL if it==FAIL;it))
...
_result_1 = (Seq[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_20 = ((it=(clas(Lookahead));next FAIL if it==FAIL;it))
(it=(_pass(true,autovar_20){it=((it=(trans());next FAIL if it==FAIL;it))
 ary_1||=[];_append(ary_1,it)
neg_1 = ( self['neg']) });next FAIL if it==FAIL;it)
_result_1 = (Lookahead[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_21 = ((it=(clas(Comment));next FAIL if it==FAIL;it))
...
_result_1 = (Comment[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_23 = ((it=(clas(Many));next FAIL if it==FAIL;it))
(it=(_pass(true,autovar_23){it=((it=(trans());next FAIL if it==FAIL;it))
 ary_1||=[];_append(ary_1,it)
o_1 = ( self['o']) });next FAIL if it==FAIL;it)
_result_1 = (Many[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_24 = ((it=(clas(Apply));next FAIL if it==FAIL;it))
...
_result_1 = (Apply[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_25 = ((it=(clas(Set));next FAIL if it==FAIL;it))
...
_result_1 = (Set[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_26 = ((it=(clas(Act));next FAIL if it==FAIL;it))
(it=(_pass(true,autovar_26){ary_1 = ((it=(args());next FAIL if it==FAIL;it))
pred_1 = ( self['pred']) });next FAIL if it==FAIL;it)
_result_1 = (Act[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_27 = ((it=(clas(Result));next FAIL if it==FAIL;it))
...
_result_1 = (Result[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_28 = ((it=(clas(Args));next FAIL if it==FAIL;it))
...
_result_1 = (Args[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_30 = ((it=(clas(Key));next FAIL if it==FAIL;it))
(it=(_pass(true,autovar_30){name_1 = ( self['name'])});next FAIL if it==FAIL;it)
_result_1 = (Key[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) },proc{autovar_31 = ((it=(clas(Pass));next FAIL if it==FAIL;it))
...
_result_1 = (Pass[ {:name=>name_1,:parent=>parent_1,:rules=>rules_1,:args=>args_1,:body=>body_1,:locals=>locals_1,:ary=>ary_1,:neg=>neg_1,:o=>o_1,:expr=>expr_1,:append=>append_1,:pred=>pred_1,:vars=>vars_1,:to=>to_1,:var=>var_1,:enter=>enter_1 }]) }));return FAIL if it==FAIL;it)
(_result_1)  
end
def transfn()
 _result_1 = ((nil))
...
_result_1 = (("#{name_1}[#{argss_1} {#{vars_1.map{|l| ":#{l[0]}=>#{l[0]}_#{l.number}" }*","} }]")) },proc{autovar_27 = ((it=(clas(Key));next FAIL if it==FAIL;it))
