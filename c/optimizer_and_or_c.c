...
ISET(src,BGET(autovar_1)); ISET(input,INT2FIX(0));
 ...
 BSET(autovar_3,BGET(it)); BSET(it,CALL(trans_AmethystOptimizer2cb_3,1,bind));  if (IGET(stop)!=Qnil){{oldinput3=IGET(input);goto break1;} } } break1: ISET(stop,Qnil);  ISET(input,oldinput3); BSET(it,BGET(autovar_2));
 BSET(ary_1,BGET(it)); 
...
ISET(src,BGET(autovar_4)); ISET(input,INT2FIX(0));
 ...
 BSET(autovar_6,BGET(it)); BSET(it,CALL(trans_AmethystOptimizer2cb_10,1,bind));  if (IGET(stop)!=Qnil){{oldinput6=IGET(input);goto break2;} } } break2: ISET(stop,Qnil);  ISET(input,oldinput6); BSET(it,BGET(autovar_5));
 BSET(ary_1,BGET(it)); 
