...
ISET(src,BGET(autovar_1)); ame_setpos(self,INT2FIX(0)); ame_setlen(self,rb_funcall(IGET(src),rb_intern("size"),0));
 ...
 BSET(autovar_2,it); VALUE oldpos3; while(1){oldpos3=ame_getpos(self);   it=CALL(transfn,0 ); FAILTEST(break1);
...
it=CALL(trans_AmethystOptimizer2cb_4,1,bind); VALUE oldpos4=ame_getpos(self);
...
alt1_2: ame_setpos(self,oldpos1);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto fail;}
 ...
 BSET(autovar_4,it); VALUE oldpos5=ame_getpos(self); VALUE oldlen2=ame_getlen(self); VALUE oldsrc2=IGET(src); int fail2=0;
ISET(src,BGET(autovar_4)); ame_setpos(self,INT2FIX(0)); ame_setlen(self,rb_funcall(IGET(src),rb_intern("size"),0));
 ...
 BSET(autovar_5,it); VALUE oldpos6; while(1){oldpos6=ame_getpos(self);   it=CALL(trans,0 ); FAILTEST(break2);
...
it=CALL(trans_AmethystOptimizer2cb_11,1,bind); VALUE oldpos7=ame_getpos(self);
