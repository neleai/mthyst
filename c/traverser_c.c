...
VALUE oldinput1; while(1){oldinput1=ame_getpos(self);   it=CALL(anything,0 ); FAILTEST(break1); if (IGET(stop)!=Qnil){{oldinput1=ame_getpos(self);goto break1;} } } break1: ISET(stop,Qnil);  ame_setpos(self,oldinput1); it=CALL(traverse_Traversercb_1,1,bind); 
...
ISET(src,BGET(autovar_2)); ame_setpos(self,INT2FIX(0));
 VALUE oldinput4; while(1){oldinput4=ame_getpos(self);   it=CALL(anything,0 ); FAILTEST(break2);
 BSET(autovar_3,it); VALUE oldinput5=ame_getpos(self);  VALUE oldsrc3=IGET(src); int fail3=0;
ISET(src,BGET(autovar_3)); ame_setpos(self,INT2FIX(0));
 ...
 if (IGET(stop)!=Qnil){{oldinput4=ame_getpos(self);goto break2;} } } break2: ISET(stop,Qnil);  ame_setpos(self,oldinput4); 
...
ISET(src,BGET(autovar_1)); ame_setpos(self,INT2FIX(0));
 ...
 BSET(autovar_2,it); VALUE oldinput3; while(1){oldinput3=ame_getpos(self);   it=CALL(traverse_item,0 ); FAILTEST(break1);
 BSET(autovar_3,it); it=CALL(traverse_item_Traversercb_4,1,bind);  if (IGET(stop)!=Qnil){{oldinput3=ame_getpos(self);goto break1;} } } break1: ISET(stop,Qnil);  ame_setpos(self,oldinput3); it=BGET(autovar_2);
 BSET(_result_1,it); 
	if (CALL(eof,0)==failobj) goto pass1;
	goto success1;
	pass1: fail1=1;
