...
VALUE oldinput1; while(1){oldinput1=IGET(input);   BSET(it,CALL(anything,0 )); FAILTEST(break1); if (IGET(stop)!=Qnil){{oldinput1=IGET(input);goto break1;} } } break1: ISET(stop,Qnil);  ISET(input,oldinput1); BSET(it,CALL(traverse_Traversercb_1,1,bind)); 
 BSET(this_1,BGET(it)); BSET(it,CALL(traverse_Traversercb_2,1,bind)); 
...
ISET(src,BGET(autovar_2)); ISET(input,INT2FIX(0));
 VALUE oldinput4; while(1){oldinput4=IGET(input);   BSET(it,CALL(anything,0 )); FAILTEST(break2);
 BSET(autovar_3,BGET(it)); VALUE oldinput5=IGET(input);  VALUE oldsrc3=IGET(src); int fail3=0;
ISET(src,BGET(autovar_3)); ISET(input,INT2FIX(0));
 ...
 BSET(it_1,BGET(it)); BSET(it,CALL(traverse_Traversercb_3,1,bind)); 
 ...
 if (IGET(stop)!=Qnil){{oldinput4=IGET(input);goto break2;} } } break2: ISET(stop,Qnil);  ISET(input,oldinput4); 
	...
	if(fail1) goto fail;
BSET(it,CALL(traverse_Traversercb_4,1,bind)); 
