...
alt1_1: ame_setpos(self,oldpos2);if (IGET(cut)!=Qnil) {ISET(cut,Qnil); goto break1;}
 ...
 BSET(autovar_2,it); VALUE oldpos3=ame_getpos(self);  VALUE oldsrc1=IGET(src); int fail1=0;
...
ISET(src,BGET(autovar_2)); ame_setpos(self,INT2FIX(0));
 ...
 BSET(autovar_3,it); VALUE oldpos4=ame_getpos(self);  VALUE oldsrc2=IGET(src); int fail2=0;
ISET(src,rb_ary_new3(1,BGET(autovar_3))); ame_setpos(self,INT2FIX(0));
   it=CALL(anything,0 ); FAILTEST(pass2);
 BSET(autovar_4,it); VALUE oldpos5=ame_getpos(self);  VALUE oldsrc3=IGET(src); int fail3=0;
ISET(src,BGET(autovar_4)); ame_setpos(self,INT2FIX(0));
 ...
 BSET(autovar_6,it); VALUE oldpos7=ame_getpos(self);  VALUE oldsrc4=IGET(src); int fail4=0;
...
it=CALL(itrans_Analyze_Variables2cb_6,1,bind);  if (IGET(stop)!=Qnil){{oldpos6=ame_getpos(self);goto break2;} } } break2: ISET(stop,Qnil);  ame_setpos(self,oldpos6); it=BGET(autovar_5);
 BSET(rules_1,it); 
	...
	goto success3;
	pass3: fail3=1;
	success3: ISET(src,oldsrc3); ame_setpos(self,oldpos5);
	if(fail3) goto pass2;
